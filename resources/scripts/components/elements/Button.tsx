...
const ButtonStyle = styled.button<Omit<Props, 'isLoading'>>`
    ...
    ${props =>
        props.color === 'grey' &&
        css`
            ${tw`border-neutral-600 bg-zinc-500 text-zinc-50`};

            &:hover:not(:disabled) {
                ${tw`bg-zinc-600 border-neutral-700`};
            }
        `};
    ...
    ${props =>
        props.isSecondary &&
        css<Props>`
            ${tw`border-neutral-600 bg-transparent text-zinc-200`};

            &:hover:not(:disabled) {
                ${tw`border-neutral-500 text-zinc-100`};
